from datos_preparados_12 import datos_preparados
...
class neurona12():
    ...
    def creacion_red_neuronal(self):
        ...
        def red_neuronas_multicapa(pesos, peso_sesgo):

            #Cálculo de la activación de la primera capa
            primera_activacion = tf.sigmoid(tf.matmulprep(datos_preparados.neuEntrada(), pesos['capa_entrada_hacia_oculta']) + peso_sesgo['peso_sesgo_capa_entrada_hacia_oculta'])
    ...
    #---------------------------------------------
        ...
        for i in range(0,datos_preparados.train_test(0).shape[0]):
            ...
            accuracy_run = sesion.run(formula_precision, feed_dict={datos_preparados.neuEntrada(): datosSonar, prepracion_datos.varReales(): clasificacionEsperada})
...
def main():
    ...
    prueba_neurona12=neurona12(tasa_aprendizaje, epochs , red)
