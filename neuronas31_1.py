...
class neurona31():
    ...
    def aprendizaje(self):
        ...
        def red_neuronas_multicapa():
            ...
            primera_activacion = tf.sigmoid(tf.matmul(datos_preparados.neuEntrada(), datos_preparados.pesos()['capa_entrada_hacia_oculta']) +
                                                                    datos_preparados.peso_sesgo()['peso_sesgo_capa_entrada_hacia_oculta'])

            #Cálculo de la activación de la segunda capa
            activacion_capa_oculta = tf.sigmoid(tf.matmul(primera_activacion, datos_preparados.pesos()['capa_oculta_hacia_salida']) +
                                                                    datos_preparados.peso_sesgo()['peso_sesgo_capa_oculta_hacia_salida'])
        ...
        for i in range(self.epochs):
            ...
            sesion.run(optimizador, feed_dict = {datos_preparados.neuEntrada():
                                                            datos_preparados.preparacion_de_datos( 0),
                                                            datos_preparados.varReales():datos_preparados.preparacion_de_datos( 2)})

            #Calcular el error
            MSE = sesion.run(funcion_error, feed_dict = {datos_preparados.neuEntrada():
                                                            datos_preparados.preparacion_de_datos( 0),
                                                            datos_preparados.varReales():datos_preparados.preparacion_de_datos( 2),})
        ...
        for i in range(0,datos_preparados.preparacion_de_datos(1).shape[0]):
            ...
            print(i,"Clase esperada: ", int(sesion.run(datos_preparados.varReales()[i][1],feed_dict={datos_preparados.varReales():
                                                                    datos_preparados.preparacion_de_datos( 3)})), " Clasificación: ", prediccion_run[0] )
        ...
        for i in range(0,207):
            ...
            prediccion_run = sesion.run(clasificaciones, feed_dict={datos_preparados.neuEntrada():datos_preparados.preparacion_de_datos( 5)[i].reshape(1,60)})
            accuracy_run = sesion.run(formula_precision, feed_dict={datos_preparados.neuEntrada():datos_preparados.preparacion_de_datos( 5)[i].reshape(1,60),
                                                                    datos_preparados.varReales():
                                                                            datos_preparados.preparacion_de_datos( 4)[i].reshape(1,2)})
...
def main():
    ...
    neurona31=neurona31(tasa_aprendizaje, epochs)
