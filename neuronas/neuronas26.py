...
import preparacion_neuronas.preparacion_datos_26 as preparacion_datos_26
import pandas as pd
...
class neurona26():
    ...
    def creacion_red_neuronal(self):
        ...
        # observaciones = pd.read_csv("datas/sonar.all-data.csv")
        # X = observaciones[observaciones.columns[0:60]].values()
        # y = observaciones[observaciones.columns[60]]

        #datos_preparados=datos_preparados( observaciones,X, y)

        def red_neuronas_multicapa():

            #Cálculo de la activación de la primera capa
            primera_activacion = tf.math.sigmoid(tf.linalg.matmul(preparacion_datos_26.datos_preparados.neuEntrada(),
                                                                    preparacion_datos_26.datos_preparados.pesos()
                                                                    ['capa_entrada_hacia_oculta']) +
                                                                    preparacion_datos_26.datos_preparados.peso_sesgo()
                                                                    ['datos_preparados.peso_sesgo()_capa_entrada_hacia_oculta'])
            ...
            activacion_capa_oculta = tf.math.sigmoid(tf.linalg.matmul(primera_activacion,
                                                                        ...
                                                                        preparacion_datos_26.datos_preparados.peso_sesgo()
                                                                        ['datos_preparados.peso_sesgo()_capa_oculta_hacia_salida'])
        ...
        funcion_error = tf.reduce_sum(tf.math.pow(preparacion_datos_26.datos_preparados.varReales()-red,2))

        #Descenso de gradiente con una tasa de aprendizaje fijada a 0,1
        optimizador = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=
                                                                    self.tasa_aprendizaje).minimize(funcion_error)
        ...
        for i in range(self.epochs):
            ...
            sesion.run(optimizador, feed_dict = {preparacion_datos_26.datos_preparados.neuEntrada():
                                                                preparacion_datos_26.datos_preparados.train_test(self,0),
                                                                preparacion_datos_26.datos_preparados.varReales():
                                                                preparacion_datos_26.datos_preparados.train_test(self,2)})

            #Calcular el error
            MSE = sesion.run(funcion_error, feed_dict = {preparacion_datos_26.datos_preparados.neuEntrada():
                                                            preparacion_datos_26.datos_preparados.train_test(self,0),
                                                            preparacion_datos_26.datos_preparados.varReales():
                                                            preparacion_datos_26.datos_preparados.train_test(self,2)})
        ...
        formula_calculo_clasificaciones_correctas = tf.equal(clasificaciones,
                                                            tf.math.argmax(preparacion_datos_26.datos_preparados.varReales(),1))
        ...
        for i in range(0,preparacion_datos_26.datos_preparados.train_test(self,1).shape[0]):

            #Se recupera la información
            datosSonar = preparacion_datos_26.datos_preparados.train_test(self,1)[i].reshape(1,60)
            clasificacionEsperada = preparacion_datos_26.datos_preparados.train_test(self,3)[i].reshape(1,2)

            # Se realiza la clasificación
            prediccion_run = sesion.run(clasificaciones, feed_dict={preparacion_datos_26.datos_preparados.neuEntrada():datosSonar})

            #Se calcula la precisión de la clasificación con la ayuda de la fórmula establecida antes
            accuracy_run = sesion.run(formula_precision, feed_dict={preparacion_datos_26.datos_preparados.neuEntrada():
                                                                    datosSonar, preparacion_datos_26.datos_preparados.varReales():
                                                                                        clasificacionEsperada})


            #Se muestra para observación la clase original y la clasificación realizada
            print(i,"Clase esperada: ", int(sesion.run(preparacion_datos_26.datos_preparados.varReales()[i][1],
                                                            feed_dict={preparacion_datos_26.datos_preparados.varReales():
                                                                        preparacion_datos_26.datos_preparados.train_test(self,3)})),
        ...
        n_clasificaciones = 0
        n_clasificaciones_correctas = 0
        for i in range(0,preparacion_datos_26.datos_preparados.train_test(self,0).shape[0]):

            # Recuperamos la información
            datosSonar = preparacion_datos_26.datos_preparados.train_test(self,0)[i].reshape(1, 60)
            clasificacionEsperada = preparacion_datos_26.datos_preparados.train_test(self,2)[i].reshape(1, 2)
        ...
        for i in range(0,207):
            ...
            prediccion_run = sesion.run(clasificaciones, feed_dict={preparacion_datos_26.datos_preparados.neuEntrada():
                                                                    preparacion_datos_26.datos_preparados.train_test(self,5)[i].reshape(1,60)})
            accuracy_run = sesion.run(formula_precision, feed_dict={preparacion_datos_26.datos_preparados.neuEntrada():
                                                                    preparacion_datos_26.datos_preparados.train_test(self,5)[i].reshape(1,60),
                                                                    preparacion_datos_26.datos_preparados.varReales():
                                                                    preparacion_datos_26.datos_preparados.train_test(self,4)[i].reshape(1,2)})
...
def main():
    ...
    prueba_neurona26=neurona26(tasa_aprendizaje, epochs )
    prueba_neurona26.creacion_red_neuronal()
